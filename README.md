...
这是一个功能强大的Python工具集，包含多个功能模块：

## 主要模块

### 1. 基础工具模块（根目录）
- **JSON菜单数据转Excel**：将JSON格式的菜单数据转换为Excel格式
- **用户菜单权限批量导出**：通过API批量获取用户列表和对应的菜单权限，导出到Excel

### 2. Menu模块（menu/目录）
- **菜单权限管理**：完整的菜单权限获取、转换和导出功能
- **角色权限管理**：获取角色列表，关联用户权限
- **用户角色权限导出**：导出用户角色权限详情到Excel

详细说明请查看 [menu/README.md](menu/README.md)
